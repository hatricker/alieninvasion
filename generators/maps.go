...
//The value at coordination (i, j) show its neighbors at four directions
...
func GenerateMapFile(cm map[string]*CityNode, w io.Writer) {
	...
	for city, node := range cm {
		coordinations := make([]string, 0, 6)
		coordinations = append(coordinations, city)
		if node.East != nil {
			coordinations = append(coordinations, "east="+node.East.Name)
		}
		if node.West != nil {
			coordinations = append(coordinations, "west="+node.West.Name)
		}
		if node.North != nil {
			coordinations = append(coordinations, "north="+node.North.Name)
		}
		if node.South != nil {
			coordinations = append(coordinations, "south="+node.South.Name)
		}
		if len(coordinations) == 1 {
			continue
		}
		coordinations = append(coordinations, "\n")
		bufWriter.WriteString(strings.Join(coordinations, " "))
	}
	bufWriter.Flush()
}
